...
int readINT8little(FILE *f, UINT8 *i)
{
    int rc;
...
/*
 * Read a BITMAP_FILEHEADER structure.
 */
 ...
 * (At the start of a BITMAP_FILEHEADER for all functions except
 ...
 */
int readSingleImageBMP(FILE *fp, RGB **argb, UINT32 *width, UINT32 *height, int *bitcount)
{
			       ...
			       char **andMask, UINT32 *width, UINT32 *height)
{
     ...
     */
    rc = readSingleImageBMP(fp, argb, &width1, &height1, &bitcnt);
    if (rc != 0)
    {
	return rc;
    }
...
void readBmpFile(char *fname, unsigned char **image, int *width, int *height, int *bitcount)
{
	...
	rc = readSingleImageBMP(fp, &bmp, &ww, &hh, bitcount);

	if (rc == 0) {

		*image = (unsigned char *) malloc( (int) (ww*hh*3));

		for (y = 0; y < hh; y++) 
			for (x = 0; x < ww; x++){
				(*image)[((hh-y-1)*ww+x)*3] = (unsigned char)bmp[y*ww+x].red;
				(*image)[((hh-y-1)*ww+x)*3+1]  = (unsigned char)bmp[y*ww+x].green;
				(*image)[((hh-y-1)*ww+x)*3+2]  = (unsigned char)bmp[y*ww+x].blue;
			}

		printf("Read %dx%d image (%d bits per pixel)\n", ww, hh, (*bitcount));
	}
	else
