...
GLfloat chalkSpecular[] = {0.8, 0.8, 0.8, 1.0};
GLfloat chalkDiffuse[] = {0.75164, 0.3, 0.0, 1.0};
GLfloat chalkAmbient[] = {0.5, 0.5, 0.5, 1.0};
GLfloat chalkShininess[] = {100.0};

GLfloat shineSpecular [] = {0.628281, 0.555802, 0.366065, 1.0};
GLfloat shineDiffuse [] = {1.0, 0.3, 0.0, 1.0};
GLfloat shineAmbient [] = {0.3, 0.3, 0.3, 1.0};
GLfloat shineShininess [] = {90.2};
...
static GLint axis = 2;
static GLdouble viewer[]= {0.0, 0.0, 100.0}; /* initial viewer location */
...
/* Update viewer position in modelview matrix */
	...
	for (i = 0; i < total_faces; i++) {
		...
		if (i == 0) 
			glTexImage2D(GL_TEXTURE_2D, 0, 3, cloth_width, 
				cloth_height, 0, GL_RGB, GL_UNSIGNED_BYTE, cloth_image);
		else if (i == 287) 
			...
			glTexImage2D(GL_TEXTURE_2D, 0, 3, hair_width, 
				hair_height, 0, GL_RGB, GL_UNSIGNED_BYTE, hair_image);
		else if (i == 2593) 
...
void load_texture()
{
	readBmpFile("cloth.bmp", &cloth_image, &cloth_width, &cloth_height, &bitcount);
	...
	readBmpFile("hair.bmp", &hair_image, &hair_width, &hair_height, &bitcount);
...
void myReshape(int w, int h)
{
	...
	gluPerspective(90.0, (float) w/ h, 2.0, 2000.0); 
